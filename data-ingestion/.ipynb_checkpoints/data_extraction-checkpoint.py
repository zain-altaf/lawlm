...
citation_types = config['vectorization']['citation_types']
...
def fetch_docket_page(cursor, existing_docket_ids, court, page_count):
    ...
    try:
        ...
        logger.info(f"Page {page_count}: fetched {len(filtered_dockets)} new dockets (after filtering duplicates)")
        return filtered_dockets, next_cursor
...
def get_chunks(docket_opinions):
    ...
    for op in docket_opinions:  # op is an Opinion
...
def vectorize_chunks(chunks, embedder):
    """Create vector embeddings for Chunk objects.
    
    Args:
        chunks: List of Chunk class instances
        embedder: SentenceTransformer model
        
    Returns:
        numpy.ndarray: Dense embeddings
    """
    if not chunks:
        return None

    # We use the raw text from the chunk for embedding
    texts_to_embed = [chunk.text for chunk in chunks]

    embeddings = embedder.encode(
        texts_to_embed,
        batch_size=min(len(texts_to_embed), 16),
...
def upload_to_qdrant(chunks, embeddings, qdrant_client, collection_name):
    """Upload vectorized Chunk objects to Qdrant using internal class dicts."""
    ...
    try:
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            # Generate UUID from the Chunk's custom ID property
            point_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, chunk.id))
            
            # Use the Chunk's built-in dictionary method
            payload = chunk.to_dict()
            
            # Add final tracking metadata
            payload['time_ingested'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        return {'status': 'success', 'vectors_created': len(points)}

    except Exception as e:
        logger.error(f"Error uploading to Qdrant: {e}")
        return {'status': 'error', 'vectors_created': 0, 'error': str(e)}
...
def ingestion(num_pages=1, court=court):
    ...
    existing_docket_ids, cursor = qdrant_client.get_existing_ids_and_cursor()
